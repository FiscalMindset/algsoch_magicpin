...
HOW TO USE:
1. Edit the iCONFIGURATION section below (lines 25-45)
...
BOT_URL='https://algsoch-magicpin.onrender.com'
...
LLM_MODEL = "phi3:latest"  # <-- Groq model (updated from decommissioned model)
...
class JudgeSimulator:
    ...
    def _warmup(self) -> bool:
        ...
        version = int(time.time())  # Use timestamp for unique version
        for slug, cat in self.dataset.categories.items():
            data, err, _ = self.client.push_context("category", slug, version, cat)
